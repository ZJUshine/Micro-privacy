...
df = pd.read_table('../../anony_jMetalPy/tools/test-clean-trans-test.txt', sep=',')
rows = df.values
...
for i,x in enumerate(x_stoi_list):
    ...
    for row in rows:
        ...
        codec = CELP(wave_path = wave_path,save_path=f'/mnt/lxc/Micro-privacy_data/eval/stoi/{audio_name}.flac',anonypara=x)
        _,lsfs,modif_lsfs = codec.run()
        orig_data, sr = sf.read(wave_path)
        code_data, sr = sf.read(f'/mnt/lxc/Micro-privacy_data/eval/stoi/{audio_name}.flac')
        ...
        asr_result = asr_model.transcribe_file(f'/mnt/lxc/Micro-privacy_data/eval/stoi/{audio_name}.flac')
        wer = fastwer.score([asr_result], ref)
        print(f"name:{audio_name},score:{score},stoi:{stoi_value},wer:{wer}")
        with open(f'evaluation_stoi_test_{i}.txt', "a") as f:
